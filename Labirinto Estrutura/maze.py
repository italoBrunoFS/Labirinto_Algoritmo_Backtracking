...
class Maze:
    ...
    def backtracking(self):
        pilha = Pilha(40 * 40) # Tamanho Max da Pilha
        pos_inicial = self.get_init_pos_player()
        pilha.push(pos_inicial)
        
        while not pilha.esta_vazia():
            pos_atual = pilha.pop()
            
            if self.find_prize(pos_atual):
                print("Caminho encontrado!")
                return True
            else:
                self.M[pos_atual] = Maze.PLAYER
                
                vizinhos = [(pos_atual[0] - 1, pos_atual[1]),
                            (pos_atual[0] + 1, pos_atual[1]),
                            (pos_atual[0], pos_atual[1] - 1),
                            (pos_atual[0], pos_atual[1] + 1)]
                
                for vizinho in vizinhos:
                    if self.is_free(vizinho):
                        pilha.push(vizinho)
            time.sleep(0.05) # Velocidade do jogador
